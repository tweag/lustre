...
lnet_ni_tq_credits(lnet_ni_t *ni)
{
	...
	credits = ni->ni_net->net_maxtxcredits / ni->ni_ncpts;
	credits = max(credits, 8 * ni->ni_net->net_peertxcredits);
	credits = min(credits, ni->ni_net->net_maxtxcredits);
...
static int
lnet_startup_lndni(struct lnet_ni *ni)
{
	int			rc = -EINVAL;
	...
	if (net->net_lnd->lnd_type == LOLND) {
		...
		the_lnet.ln_loni = ni;
		ni->ni_net->net_peertxcredits = 0;
		ni->ni_net->net_peerrtrcredits = 0;
		ni->ni_net->net_maxtxcredits = 0;
		ni->ni_net->net_peer_timeout = 0;
		return 0;
	}

	if (ni->ni_net->net_peertxcredits == 0 ||
	    ni->ni_net->net_maxtxcredits == 0) {
	...
	CDEBUG(D_LNI, "Added LNI %s [%d/%d/%d/%d]\n",
		...
		ni->ni_net->net_peerrtrcredits, ni->ni_net->net_peer_timeout);
...
static int
lnet_startup_lndnet(struct lnet_net *net)
{
	struct lnet_ni		*ni;
	...
	__u32			lnd_type;
	lnd_t			*lnd;
	int			peer_timeout = net->net_peer_timeout;
	int			maxtxcredits = net->net_maxtxcredits;
	int			peerrtrcredits = net->net_peerrtrcredits;
	...
	while (!list_empty(&net->net_ni_added)) {
		...
		LASSERT(ni->ni_net->net_peer_timeout <= 0 ||
	...
	if (net_l != net && net_l != NULL) {
		...
		if (maxtxcredits != -1)
			net->net_maxtxcredits = maxtxcredits;
		if (peerrtrcredits != -1)
			net->net_peerrtrcredits = peerrtrcredits;
		  ...
		  struct lnet_ioctl_net_config *net_config)
{
	...
	*nid = ni->ni_nid;
	*peer_timeout = ni->ni_net->net_peer_timeout;
	*peer_tx_credits = ni->ni_net->net_peertxcredits;
